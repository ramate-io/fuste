...
impl<const MEMORY_SIZE: usize> ExecutableInstruction<MEMORY_SIZE> for Sra {
	#[inline(always)]
	fn execute(self, machine: &mut Machine<MEMORY_SIZE>) -> Result<(), ExecutableInstructionError> {
		...
		// Perform arithmetic right shift (only use lower 5 bits of rs2 for shift amount)
...
mod tests {
	...
	fn test_sra_with_negative_number() -> Result<(), ExecutableInstructionError> {
		...
		assert_eq!(machine.registers().get(3), 0xC0000000); // 0x80000000 >> 1 = 0xC0000000
