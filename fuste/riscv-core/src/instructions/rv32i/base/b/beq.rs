...
impl<const MEMORY_SIZE: usize> ExecutableInstruction<MEMORY_SIZE> for Beq {
	#[inline(always)]
...
mod tests {
	...
	#[test]
	fn test_beq_inner_construction() -> Result<(), ExecutableInstructionError> {
		...
		let instruction = Beq::new(B::new(0b000, 1, 2, 8)); // branch offset 8
	...
	#[test]
	fn test_beq_branch_not_taken() -> Result<(), ExecutableInstructionError> {
		let mut machine = Machine::<1024>::new();
	...
	#[test]
	fn test_beq_with_negative_offset() -> Result<(), ExecutableInstructionError> {
		let mut machine = Machine::<1024>::new();
		...
		let instruction = Beq::new(B::new(0b000, 1, 2, -4)); // branch offset -4
		instruction.execute(&mut machine)?;
	...
	#[test]
	fn test_beq_from_word() -> Result<(), ExecutableInstructionError> {
		let mut machine = Machine::<1024>::new();
		...
		// Create BEQ instruction word: BEQ x1, x2, 12
		let b = B::new(0b000, 1, 2, 12);
		let word = b.to_word(0b1100011);
		let instruction = Beq::from_word(word);
		instruction.execute(&mut machine)?;

		// Check PC was set to 0 + 12 = 12 (branch taken)
		assert_eq!(machine.registers().program_counter(), 12);
